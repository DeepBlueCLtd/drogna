/**
 * Feature 108: shore advisories authored on cadence, stored append-only, and staged.
 * Nothing mocked: the source authors from its seed stream on the real clock, the store
 * refuses at its own seam, and every document is validated against its master.
 *
 * The replay and release-scoring scenarios live in advisories-release.test.ts, split for
 * the reason advisories.fixture.ts records.
 */
import { describe, expect, it } from 'vitest';
import { schemaDocuments } from '../../generated/schema-documents.js';
import type { FeaturesResponseCollections, FeaturesResponseFeatureCollection } from '../../generated/types.js';
import { buildBackend, drive, lockstepConfig, options, validator } from './advisories.fixture.js';

/** Every `type: "string"` node in a master, so the closed-vocabulary claim can be held. */
