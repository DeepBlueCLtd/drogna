...
const COMPONENT_STEPS: Record<string, { title: string; what: string; panel: string }> = {
  ...
  'snapshot-source': {
    ...
    what: 'Where the ocean and the forecasts came from. Every situation declares four eras — the archive and the now-cast from the generator above, the analyses from the analyst and the forecast instances from the model runner — and they are usually authored ahead of time, from that situation’s own recorded seed, and committed as a build artefact, because computing them again on every visit cost several seconds and produced the same bytes. Whether this run took that path is what the face below says: it republishes what the artefact held, or reports that it found none and the run computed its own.',
...
const FORECAST_STEPS: SubjectStep[] = [
  ...
  },
  {
    ...
    panel: 'Pick a square and the region answers for that column. The map above is the four provenance shares at a chosen depth, every square a cell the analyst published rather than an average of several \u2014 and it can be approached: come closer and the cells drawn are the ones in view, at the resolution the analyst filed them. The lines run to the instruments whose observations the gain actually weighed, each as wide as it counted; a source that reached the column but contributed nothing at the chosen depth keeps its place as a hollow marker rather than vanishing. Beneath, the depth profile stacks each level’s composition and prints every figure, including the part of the correction that came through a reading beyond this cell’s own reach and so has no line to be drawn from. Opening the volume puts that plan in its setting: the field drawn semi-transparent level by level, and the thermocline as a surface through it — placed per column rather than as the one depth the run publishes, so where a feature displaces the layer the surface follows it. The caption counts how many depths it found and how level the field is, and says what it cannot tell from them.',
  },
  {
    subject: 'ahead',
