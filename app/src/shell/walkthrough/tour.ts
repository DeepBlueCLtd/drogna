...
const FORECAST_STEPS: SubjectStep[] = [
  ...
  },
  {
    ...
    panel: 'Pick a square and the region answers for that column. The map above is the four provenance shares at a chosen depth, every square a cell the analyst published rather than an average of several \u2014 and the field is finer than a panel is wide, so it can be approached: come closer and the cells drawn are the ones in view, at the resolution the analyst filed them. The lines run to the instruments whose observations the gain actually weighed, each as wide as it counted; a source that reached the column but contributed nothing at the chosen depth keeps its place as a hollow marker rather than vanishing. Beneath, the depth profile stacks each level’s composition and prints every figure, including the part of the correction that came through a reading beyond this cell’s own reach and so has no line to be drawn from. Opening the volume puts that plan in its setting: the field drawn semi-transparent level by level, and the thermocline as a surface through it — placed per column, so it domes and tilts where the features move it, which a single published depth cannot show.',
  },
  {
    subject: 'ahead',
