...
const COMPONENT_STEPS: Record<string, { title: string; what: string; panel: string }> = {
  ...
  'snapshot-source': {
    ...
    what: 'Where the ocean and the forecasts came from. For most situations all four eras — the archive and the now-cast from the generator above, the analyses from the analyst and the forecast instances from the model runner — were authored ahead of time, from this situation’s own recorded seed, and committed as a build artefact, because computing them again on every visit cost several seconds and produced the same bytes. This component republishes them, and the loop takes over live within a few ticks of the console opening.',
