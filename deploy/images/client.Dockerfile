...
# The topology matrix (018) and the topic tree (022) both import the generated artefact
# the same way schemas.ts imports the masters — `../../../contracts/topology.json` — so it
# must sit beside `schemas/` in the reproduced layout. The dockerignore's exception admits
# it to the context; this line is the other half, and the build fails on the import if
# either half is missing. Both lanes met that failure independently.
