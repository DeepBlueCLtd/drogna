...
# The database owner's password, generated by ensure_secrets before this module runs.
# Deliberately not in SECRET_NAMES: that tuple is the list the shell generates from, and
# this value is generated separately because Postgres reads it as POSTGRES_PASSWORD at
# first start rather than from a password file this module writes.
DATABASE_SECRET = "HARNESS_DATABASE_PASSWORD"
...
def _with_database_secret(dsn: str, owner: str, secret: str, *, source: Path) -> str:
    ...
    up: the tracked files carry the role and never the secret. It was missing, and the
    consequence was the observation store's exact analogue of the broker gap ADR-0016
...
def render_destination(destination: str, values: dict[str, str], root: Path | None = None) -> Path:
    ...
    for source in sorted(source_dir.iterdir()):
        ...
        # DSNs are walked rather than read from a known key: the broker's URL is always at
...
def main(argv: list[str] | None = None) -> int:
    ...
    values = {name: os.environ.get(name, "") for name in (*SECRET_NAMES, DATABASE_SECRET)}
    try:
