...
| The read path's other half | the proxy's upstream still names `/query/collections/…`, which 404s, and releases two names the query layer does not serve — `spikes/map-to-ocean/FINDING.md` | lane C's fix corrected the advertised links and closed the item; this half was never in that reading, and now has no owner |
| The browser cannot reach `/released` | locally the client, proxy and clock are three origins, with no CORS and a deliberate Basic challenge — the owner's topology decision, not a lane's | the map, the route's conditions and the rate control all fetch through it; the droplet's single origin does not have the problem |
