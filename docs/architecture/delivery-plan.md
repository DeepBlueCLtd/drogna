...
| 014 | Offload export | C-17, FR-43, FR-44 | Delivered. ~~Only T047-geometry's producer half open~~ closed 28 Aug (lane K): the geometry is staged beside the bundle as a run-manifest sibling, per the decided shape |
...
| K — residue | `libs/harness_core/`, `tests/`, `services/offload/` (T047 only) | **Delivered 28 August (PR #45)**: 001 T033/T042/T047 — the two-participant byte-identical lockstep replay scenario, AT-04 upgraded from the weaker claim and watched failing against three planted violations, `scripts/replay_proof.py` as the one-command proof, and the TypeScript/SQL gate fixtures. 014 T047 per the decided shape: the geometry is staged beside the bundle as a run-manifest sibling the sidecar names without membership, SC-006 unchanged. 003 T040 and 004 T044's droplet halves remain, and join once lane I makes the droplet real |
