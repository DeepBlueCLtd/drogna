...
| The browser cannot reach `/released` | the page is served off the proxy's origin and the released prefix answers 401 with no CORS — **decided 28 August**: serve the page through the proxy, one door, one clearance (`spikes/map-to-ocean/FINDING.md`) | the map and the route's conditions both fetch through it. The clock is fine — it opts into cross-origin and works. The droplet is worse, not better: its proxy fronts neither the page nor the clock, so the address `public_url` advertises has never served anything |
