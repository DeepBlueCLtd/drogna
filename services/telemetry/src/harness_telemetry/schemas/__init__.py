...
The telemetry *message* contract does not travel. Arriving payloads are validated against
the Pydantic model generated from ``contracts/schemas/telemetry.schema.json`` into
``libs/harness_types``, which is the same definition every producer on this branch holds
(Constitution III), so a producer and this consumer cannot come to disagree about what a
valid telemetry message is. That is the convention ``generators.toml`` already records for
the observation master, and shipping a second copy of a document the generated model is
made from would create exactly the second definition it exists to prevent.
...
CONFIG_SCHEMA = "config.telemetry.schema.json"
