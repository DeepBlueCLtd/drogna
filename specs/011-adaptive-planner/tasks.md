...
- [ ] T001 Create the `services/planner/` package skeleton — `pyproject.toml`,
...
- [ ] T002 Write `contracts/schemas/plan.schema.json` — committed route with
...
- [ ] T003 [P] Write `contracts/schemas/config.planner.schema.json`, `$ref`ing
...
- [ ] T004 Regenerate Python and TypeScript types from `plan.schema.json` and confirm
  the drift check passes (`libs/harness_types/`, `client/src/generated/`).
- [ ] T005 [P] Add `config/local/planner.json` and `config/droplet/planner.json` —
  same shape, destination-specific values.
- [ ] T006 [P] Contract test asserting the plan schema accepts a canonical example,
...
- [ ] T007 [P] Implement the planning cell — H3 index at the configured resolution
...
- [ ] T009 [P] [US1] Unit test for the sensing footprint: which cells a visit informs
  and by how much, in the horizontal and in depth, against hand-computed values, in
  `services/planner/tests/test_sensing.py`.
- [ ] T010 [P] [US1] Unit test for collapse and regrowth: a visit reduces the informed
...
- [ ] T011 [P] [US1] Unit test for the collapse-aware value on the near/far geometry,
...
- [ ] T012 [US1] Implement config loading and schema validation as the first
...
- [ ] T016 [P] [US2] Unit test for selection under budget: route cost within budget,
  candidate cells left unvisited, no requirement that all be visited, in
  `services/planner/tests/test_select.py`.
- [ ] T017 [P] [US2] Determinism test: the same field, budget and seed produce the
...
- [ ] T018 [P] [US2] Test for the empty-route cases: budget too small for any
...
- [ ] T019 [P] [US2] Forbidden-vocabulary test over `contracts/schemas/plan.schema.json`
...
- [ ] T020 [US2] Implement prize-collecting selection under a budget — greedy
...
- [ ] T021 [US2] Assemble the committed route: ordering, per-vertex H3 index, depth
...
- [ ] T022 [US2] Implement plan publication on `ctl/plan` and the heartbeat carrying
  `planning` / `no-field` / `nothing-worth-sampling`, in
  `services/planner/src/harness_planner/publish.py`.
- [ ] T023 [US2] Wire the service: subscribe to `ctl/run-published` and `obs/#`, read
...
- [ ] T024 [P] [US3] Unit test for the commitment rule: the committed prefix is
...
- [ ] T025 [P] [US3] Integration test: a new uncertainty field triggers a replan
...
- [ ] T026 [US3] Implement the receding horizon: cadence trigger in simulation time,
...
- [ ] T027 [US3] Implement the commitment window and improvement margin, with
  departure recording, in `services/planner/src/harness_planner/commitment.py`.
- [ ] T028 [US3] Extend the service to maintain last-informed times from `obs/#`, to
...
- [ ] T031 [US4] Implement forward projection of uncertainty growth and per-region
  crossing times against the usable-confidence threshold, in
  `services/planner/src/harness_planner/projection.py`.
- [ ] T032 [US4] Attach the projection report to every published plan so the output is
...
- [ ] T033 Write `docs/algorithms/informative-path-planning.md` — the value function
...
- [ ] T034 Measure the selection heuristic's optimality gap against exhaustive search
...
- [ ] T035 [P] Add the planner service to the Compose configuration with its config
...
- [ ] T036 [P] Run the wall-clock, seeded-RNG, literal-path and forbidden-vocabulary
