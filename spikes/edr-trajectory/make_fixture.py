...
def grid() -> dict[str, np.ndarray]:
    return {
        ...
        "depth": DEPTHS.copy(),
        "time": (
            T0.astype("datetime64[ns]") + np.arange(N_TIME) * np.timedelta64(TIME_STEP_HOURS, "h")
        ),
    }
    ...
    dataset = xr.Dataset(
        ...
        attrs={
            ...
            "source": (
                ...
                f"u = lon - {LON0}, v = lat - {LAT0}, w = depth/{DEPTH_SCALE}, "
                f"s = hours since {T0!s}. Coefficients: {json.dumps(coeffs, sort_keys=True)}"
            ),
            "comment": (
