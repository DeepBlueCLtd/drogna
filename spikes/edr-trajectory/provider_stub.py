...
class SpikeTrajectoryEDRProvider(XarrayEDRProvider):
    ...
    def trajectory(self, **kwargs):
        ...
        times = EPOCH + (np.array(m_values, dtype=float) * 1e9).astype(
            'timedelta64[ns]'
        )
        ...
        indexers = {
            self.x_field: xr.DataArray(longitudes, dims='vertex', coords={'vertex': index}),  # noqa: E501
            self.y_field: xr.DataArray(latitudes, dims='vertex', coords={'vertex': index}),  # noqa: E501
            'time': xr.DataArray(times, dims='vertex', coords={'vertex': index}),
        }
        if self.z_field is not None:
            indexers[self.z_field] = xr.DataArray(
        ...
        for name in fields:
            ...
            ranges[name] = {
                ...
                'values': [
                    None if value != value else float(value) for value in values
                ],
            }

        composite = [
            [_iso(times[i]), float(longitudes[i]), float(latitudes[i]), float(depths[i])]  # noqa: E501
