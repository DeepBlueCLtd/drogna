...
def nearest_step_expectation() -> np.ndarray:
    ...
    return theta(vertices["lon"], vertices["lat"], vertices["depth"], snapped, coefficients())
...
def score(body: dict) -> dict:
    ...
    return {
        ...
        "max_abs_error_vs_single_time_query_degC": error_against(expectations["single_time_query"]),
        ...
        "per_vertex_table": [
            {
                ...
                "expected_single_time_query": float(expectations["single_time_query"][index]),
...
def validate_coveragejson(body: dict) -> dict:
    ...
    if domain.get("domainType") != "Trajectory":
        problems.append(f"domainType is {domain.get('domainType')!r}, expected 'Trajectory'")
    ...
    times_sent = [str(np.datetime64(value, "s")) + "Z" for value in route()["time"]]
    ...
    return {
        ...
        "referencing_systems": [
            entry.get("system", {}).get("type") for entry in (domain.get("referencing") or [])
        ],
...
def boundary_probes(base: str, collection: str = COLLECTION) -> dict:
    ...
    cases = {
        ...
        "beyond_last_time_step": (
            f"LINESTRING ZM (-2.0 50.0 -10 {beyond_horizon}, -1.0 51.0 -10 {beyond_horizon + hour})"
        ),
        "non_monotonic_times": (
        ...
        "above_the_surface": (
            f"LINESTRING ZM (-2.0 50.0 250 {origin + 4 * hour}, -1.0 51.0 300 {origin + 8 * hour})"
        ),
    }

    results = {}
    for label, wkt in cases.items():
        response = fetch(edr_url(base, collection, "trajectory", {"coords": wkt, "f": "json"}))
        ...
        results[label] = {
            ...
            "composite": (
                ((body.get("domain") or {}).get("axes") or {}).get("composite", {}).get("values")
...
def _long_route_url(base: str, count: int) -> str:
    ...
    wkt = (
        "LINESTRING ZM ("
        + ", ".join(f"{lon[i]:.5f} {lat[i]:.5f} -25 {seconds[i]:.0f}" for i in range(count))
        + ")"
    )
...
def length_probe(base: str) -> dict:
    ...
    def attempt(count: int) -> dict:
        ...
        record = {
            ...
            "body_excerpt": (
                str(response.get("body"))[:300] if response["status"] != 200 else None
            ),
        }
...
def run() -> dict:
    summary: dict = {
        ...
        "geos_version_of_query_client": ".".join(str(part) for part in shapely.geos_version),
    ...
    for label, collection, base in (
        ...
        summary.setdefault("advertised_query_types", {})[label] = (
            sorted((body.get("data_queries") or {}).keys()) if isinstance(body, dict) else None
        )
    ...
    url = edr_url(
        ...
        COLLECTION,
        "trajectory",
        {"coords": wkt, "parameter-name": PARAMETER, "datetime": f"{QUERY_TIME}Z", "f": "json"},
    )
    response = fetch(url)
    ...
    if isinstance(response.get("body"), dict):
        ...
        summary["trajectory_at_pin"]["coveragejson"] = validate_coveragejson(response["body"])
    ...
    summary["vertical_direction"] = {
        ...
        "values_differ": (
            returned_values(descending.get("body")) != returned_values(response.get("body"))
    ...
    summary["boundary_probes"] = {
        ...
        "extrapolating_outside_the_domain": boundary_probes(AT_PIN, EXTRAPOLATING_COLLECTION),
    }
...
def _print(summary: dict) -> None:
    ...
    print(f"well-formed Trajectory    : {covjson.get('well_formed_trajectory_coveragejson')}")
    scored = at_pin.get("score") or {}
    if scored.get("scored"):
        print()
        print(f"tolerance                 : {scored['tolerance_degC']} degC")
        print(f"max error vs per-vertex   : {scored['max_abs_error_vs_per_vertex_degC']:.3e} degC")
        single_time = scored["max_abs_error_vs_single_time_query_degC"]
        nearest_step = scored["max_abs_error_vs_nearest_time_step_degC"]
        print(f"max error vs single-time  : {single_time:.3e} degC")
        print(f"max error vs nearest step : {nearest_step:.3e} degC")
        print()
    ...
    print(
        f"provider saw shapely      : {handoff.get('shapely_version')} "
        f"/ GEOS {handoff.get('geos_version')}"
    )
    print()
