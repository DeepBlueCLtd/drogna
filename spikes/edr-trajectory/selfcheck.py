...
def check_fixture_matches_analytic_form() -> dict:
    if not FIXTURE_PATH.exists():
        raise SelfCheckFailure(
            f'fixture missing at {FIXTURE_PATH}; run make_fixture.py first'
        )
    ...
    if error > TOLERANCE:
        raise SelfCheckFailure(
            f'written fixture departs from the analytic form by {error} degC, '
            f'tolerance {TOLERANCE}'
        )
    if dataset.attrs.get('drogna_synthetic') != 'true':
        raise SelfCheckFailure('fixture does not declare itself synthetic')
    return {
...
def check_interpolation_is_exact_at_route_vertices() -> dict:
    ...
    sampled = dataset['sea_water_temperature'].interp(
        ...
        depth=xr.DataArray(
            vertices['depth'], dims='vertex', coords={'vertex': dimension}
        ),
        time=xr.DataArray(
            vertices['time'], dims='vertex', coords={'vertex': dimension}
        ),
        method='linear',
    ).values
...
def run() -> dict:
    return {
        ...
        'interpolation_exact_at_vertices': (
            check_interpolation_is_exact_at_route_vertices()
        ),
