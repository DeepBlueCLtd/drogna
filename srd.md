...
- **FR-14** The shell shall be a dockable multi-panel layout with top-level tabs
  **Intro, Background, Holdings, Operator, Map, Messages** at first run,
  user-rearrangeable by drag and drop. **Amended by feature 114:** the list read
  *Intro, Background, System, Holdings, Operator, Map, Messages* until FR-63 withdrew
...
- **FR-16** The full component layout renders from day one, greyed out; illumination is
  ...
  pattern every component follows. **Amended by feature 114:** this obligation is
  ...
  list view as declared figures (FR-63).
...
- **FR-46** The **Holdings** tab shall show the coverage store's inventory as the store
  ...
  *(Constitution VII)*. **Amended by feature 114:** the inventory is presented as a
  timeline in simulation time rather than a list in arrival order (FR-64), and a derived
  comparison of a forecast instance against the truth published for the same instant
  joins the manifest view (FR-65). The refusal rule and the no-polling rule above are
...
- **FR-23** The Messages tab renders live broker traffic with inspection of the
  ...
  114:** traffic is *drawn* as well as listed (FR-66) and the inspector renders a payload
...
- **FR-24** The topic tree draws the declared topology lit by live traffic: structure
  ...
  received messages and nothing else, the two never mixing; **amended by feature 114**,
  which promotes it out of its disclosure to a primary region of the panel and makes
  selecting a node filter the traffic display and the list (FR-67), changing where it is
...
- **FR-40** The Map panel (Deck.gl) renders: the fields; the uncertainty field
  ...
  **Amended by feature 114:** the platform's historic track and demanded course are
  required in *every* projection the panel offers, the depth volume included, where the
  track is drawn at the depths the platform reported (FR-69). That is parity, not a new
  layer. The time
...
- **FR-61** The shell shall carry a **help control**, visually distinct from the
  ...
  covering a component would read as a complete tour. **Amended by feature 114
  (FR-70, ADR-0037):** the control is carried by the panel it explains rather than by the
...
- **FR-62** The walkthrough **teaches and does not report**: it shall not claim any
  ...
  **Amended by feature 114:** it is parameterised by its tour still, and is carried by
  the panel; the view-opening clause retires with the header placement.

### 5.14 The tabs beyond Operator (feature 114)
...
specification is `specs/114-engaging-tabs/`.
...
- **FR-65** The tab shall offer, for a forecast **instance** whose validity has elapsed, a
...
- **FR-69** The Map panel shall draw the platform's historic track and its demanded course
  ...
  changes.
- **FR-70** The help control shall be carried **by the panel it explains**, at that panel's
